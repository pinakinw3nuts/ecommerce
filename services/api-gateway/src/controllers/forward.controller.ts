...
function findServiceConfig(path: string): ServiceConfig | undefined {
  ...
  // Special case for the exact coupons path pattern we're having trouble with
  if (path.startsWith('/api/v1/coupons?')) {
    logger.info({
      msg: 'Special case: Exact match for /api/v1/coupons?',
      path,
    ...
    if (productServiceUrl) {
      ...
      const targetUrl = `${productServiceUrl}/api/v1/coupons${queryString}`;
      logger.info({
