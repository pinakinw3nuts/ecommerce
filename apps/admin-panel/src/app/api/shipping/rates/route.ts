...
export async function GET(request: NextRequest) {
  try {
    ...
    const url = `${SHIPPING_SERVICE_URL}/api/v1/shipping/admin/shipping-rates${search}`;
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        {
          message: errorData.message || 'Failed to fetch shipping rates',
          error: errorData,
        },
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        {
          message: errorData.message || 'Failed to create shipping rate',
          error: errorData,
        },
        { status: response.status },
      );
