...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { 
          message: errorData?.message || 'Failed to fetch brands',
    ...
    const brands = (await response.json()) as BackendBrand[];
    console.log('Backend response:', brands);

    // Filter brands based on search term if present
    ...
    const transformedData = {
      ...
      pagination: {
        ...
        currentPage: page,
        pageSize: pageSize,
        hasMore: page * pageSize < filteredBrands.length,
        hasPrevious: page > 1,
      },
    };
