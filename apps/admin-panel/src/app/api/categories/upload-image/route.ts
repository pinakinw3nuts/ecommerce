...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('Uploading image to product service...');
    
    const response = await fetch(`${PRODUCT_SERVICE_URL}/api/v1/admin/categories/upload-image`, {
      method: 'POST',
      body: formDataToSend,
      headers: {
    ...
    if (!response.ok) {
      ...
      console.error('Error uploading image to product service:', error);
