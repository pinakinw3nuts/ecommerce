...
export async function POST(request: NextRequest) {
  try {
    ...
    const { couponIds, updates } = await request.json();
    ...
    if (!updates || typeof updates !== 'object' || Object.keys(updates).length === 0) {
      return NextResponse.json(
        { message: 'No updates provided', code: 'VALIDATION_ERROR' },
        { status: 400 }
      );
    }

    const response = await makeRequest(
      `${PRODUCT_SERVICE_URL}/api/v1/admin/coupons/bulk-update`,
      {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token.value}`,
        },
        body: JSON.stringify({ couponIds, updates }),
      }
    );
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { 
          message: errorData?.message || 'Failed to update coupons',
