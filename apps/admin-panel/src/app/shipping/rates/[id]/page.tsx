...
export default function EditShippingRatePage() {
  ...
  const { data: rate, error: fetchError } = useSWR<ShippingRate>(
    id ? `shippingRate_${id}` : null,
    () => shippingApi.getShippingRate(id as string)
  );
  ...
  const onSubmit = async (data: ShippingRateFormValues) => {
    try {
      await shippingApi.updateShippingRate(id as string, data as Partial<ShippingRate>);
  ...
  if (fetchError) return <p>Failed to load shipping rate.</p>;
