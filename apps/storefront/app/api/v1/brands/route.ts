...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(`Making API request to: ${PRODUCT_SERVICE_URL}/brands`);
    ...
    if (apiData && apiData.data && Array.isArray(apiData.data)) {
      // Transform the brands data to match the expected format
      const transformedBrands = apiData.data.map((brand: any) => ({
        id: brand.id,
        name: brand.name,
    ...
    console.error('Error fetching brands from service:', error);
