...
/**
 * GET handler for /api/products/sale
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('Making API request to:', `${baseUrl}/v1/products/sale`);
    
    // Forward request to API gateway
    const response = await axios.get(`${baseUrl}/v1/products/sale`, {
      params: {
        limit,
      },
      headers: {
    ...
    if (apiData && Array.isArray(apiData.data)) {
      ...
      const transformedData = {
        products: apiData.data.map((product: any) => ({
          ...
          inStock: true,
          discountedPrice: product.salePrice || (product.price * 0.8), // 20% discount if no sale price provided
    ...
    return NextResponse.json(
      { error: 'Failed to fetch sale products', message: error.message },
