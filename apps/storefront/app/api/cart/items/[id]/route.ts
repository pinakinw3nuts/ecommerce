...
export async function PATCH(
  ...
  try {
    ...
    // Call the cart service API directly to patch the item
    const apiUrl = `${CART_SERVICE_URL}/cart/items/${itemId}?cartId=${cartId}`;
    
    console.log(`Making PATCH request to: ${apiUrl} with data:`, body);
    ...
    // After patching the item, fetch the full cart to return
    console.log(`Fetching updated cart data for cartId: ${cartId}`);
    const cartResponse = await axios.get(`${CART_SERVICE_URL}/cart?cartId=${cartId}&refresh=false`, {
      headers: {
        'Authorization': token ? `Bearer ${token}` : '',
    ...
    return NextResponse.json(
      { 
        error: 'Failed to patch cart item', 
        message: error.message,
        details: error.response?.data || null
      },
      { status: error.response?.status || 500 }
