...
export class ReviewService {
  ...
  constructor() {
    ...
    this.productRatingRepo = AppDataSource.getRepository(ProductRating);
  }
  ...
  async createReview(data: {
    ...
    logger.info({ userId: data.userId, productId: data.productId }, 'Creating new review');
    ...
    logger.info({ reviewId: savedReview.id }, 'Review created successfully');
  ...
  async getReviewById(id: string): Promise<Review | null> {
    logger.info({ reviewId: id }, 'Getting review by ID');
  ...
  async updateReview(id: string, userId: string, data: {
    ...
    if (!review) {
      logger.warn({ reviewId: id, userId }, 'Review not found or does not belong to user');
      return null;
    }
    ...
    if (updateResult.affected === 0) {
      logger.warn({ reviewId: id }, 'Review update failed');
      return null;
    }
  ...
  async deleteReview(id: string, userId: string): Promise<boolean> {
    ...
    if (deleteResult.affected === 0) {
      logger.warn({ reviewId: id }, 'Review deletion failed');
      return false;
    }
  ...
  async moderateReview(id: string, isPublished: boolean): Promise<Review> {
    try {
      ...
      logger.info(`Review ${id} ${isPublished ? 'published' : 'unpublished'} by admin`);
      
      return updatedReview;
    } catch (error) {
      logger.error(`Failed to moderate review ${id}:`, {
        error: error instanceof Error ? error.message : 'Unknown error'
      });
      throw error;
    }
  ...
  async getProductRating(productId: string): Promise<{
    ...
    logger.info({ productId }, 'Getting product rating');
  ...
  private async updateProductRating(productId: string): Promise<void> {
    try {
      logger.info({ productId }, 'Updating product rating');
