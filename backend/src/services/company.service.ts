...
const companyLogger = logger.child({ service: 'CompanyService' });
...
export class CompanyService {
  ...
  constructor() {
    ...
    this.companyProfileRepository = AppDataSource.getRepository(CompanyProfile);
  }
  ...
  async createCompany(
    ...
    return AppDataSource.transaction(async transactionalEntityManager => {
      try {
        ...
        companyLogger.info({ companyId: savedCompany.id }, 'Company created successfully');
        ...
        companyLogger.info({ companyId: savedCompany.id }, 'Company profile created');
        ...
        companyLogger.info({ companyId: savedCompany.id, userId }, 'User assigned as company owner');

        return savedCompany;
      } catch (error) {
        companyLogger.error({ error, companyData }, 'Error creating company');
        throw error;
      }
    });
  }
  ...
  async updateCompany(
    ...
    try {
      ...
      companyLogger.info({ companyId }, 'Company updated successfully');
      
      return result;
    } catch (error) {
      companyLogger.error({ error, companyId, companyData }, 'Error updating company');
      throw error;
    }
  }
  ...
  async updateCompanyProfile(
    ...
    try {
      ...
      if (!profile) {
        ...
        companyLogger.info({ companyId }, 'Company profile created');
        return savedProfile;
      }
      ...
      companyLogger.info({ companyId }, 'Company profile updated successfully');
      
      return result;
    } catch (error) {
      companyLogger.error({ error, companyId, profileData }, 'Error updating company profile');
      throw error;
    }
  }
  ...
  async getCompanyById(companyId: string): Promise<Company | null> {
    try {
      ...
      companyLogger.error({ error, companyId }, 'Error fetching company');
      throw error;
    }
  }
  ...
  async getCompanyByGst(gstNumber: string): Promise<Company | null> {
    try {
      ...
      companyLogger.error({ error, gstNumber }, 'Error fetching company by GST');
      throw error;
    }
  }
  ...
  async assignCreditLimit(
    ...
    try {
      ...
      companyLogger.error({ error, companyId, creditLimit }, 'Error assigning credit limit');
      throw error;
    }
  }
    ...
    try {
      ...
      const companyUser = this.companyUserRepository.create({
        ...
        permissions: userData.permissions || {
          canManageUsers: userData.role === CompanyRole.OWNER || userData.role === CompanyRole.ADMIN,
          canViewReports: true,
          canApproveOrders: userData.role === CompanyRole.OWNER || userData.role === CompanyRole.ADMIN || userData.role === CompanyRole.APPROVER,
          canManageProducts: userData.role === CompanyRole.OWNER || userData.role === CompanyRole.ADMIN || userData.role === CompanyRole.BUYER
        },
        invitationToken,
        invitationExpiry,
      ...
      companyLogger.info({ companyId, userId: userData.userId, role: userData.role }, 'User added to company');
      
      return savedUser;
    } catch (error) {
      companyLogger.error({ error, companyId, userData }, 'Error adding company user');
      throw error;
    }
  }
  ...
  async updateCompanyUser(
    ...
    try {
      ...
      companyLogger.info({ companyId, userId }, 'Company user updated');
      
      return updatedUser;
    } catch (error) {
      companyLogger.error({ error, companyId, userId, updates }, 'Error updating company user');
      throw error;
    }
  }
  ...
  async getAllCompanies(
    ...
    try {
      ...
      companyLogger.error({ error, page, limit, filters }, 'Error fetching companies');
      throw error;
    }
  }
  ...
  async getUserCompanies(userId: string): Promise<Company[]> {
    try {
      ...
      companyLogger.error({ error, userId }, 'Error fetching user companies');
      throw error;
    }
  }
  ...
  async getCompanyUsers(companyId: string): Promise<CompanyUser[]> {
    try {
      ...
      companyLogger.error({ error, companyId }, 'Error fetching company users');
      throw error;
    }
  }
  ...
  async removeCompanyUser(companyId: string, userId: string): Promise<boolean> {
    try {
      ...
      companyLogger.info({ companyId, userId }, 'User removed from company');
      
      return result.affected ? result.affected > 0 : false;
    } catch (error) {
      companyLogger.error({ error, companyId, userId }, 'Error removing company user');
      throw error;
    }
  }
} 